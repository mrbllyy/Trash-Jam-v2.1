  ...
  typelist=["glass","chemical","organical","medical_waste"];
  typelist.push()
...
var sort_object=(obj)=> {
  const sortable = keysSorted = Object.keys(obj).sort(function(a,b){return obj[a]-obj[b]})
  return (sortable);
}
var  KNN_=(k,new_data_point,x_train_set,y_train_set)=>{
    ...
    for (let im = 0; im < neighbours_labels.length; im++) {
      if (neighbours_labels[im] === 'glass') counter1++;
      else if (neighbours_labels[im] === 'textile') counter2++;
      else if (neighbours_labels[im] === 'organic') counter3++;
      else if (neighbours_labels[im] === 'medical') counter4++;
    }
    var retList={};
    var greatestVal=0;
